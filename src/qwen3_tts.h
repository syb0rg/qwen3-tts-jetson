...
public:
    ...
    // Encode speaker embedding from reference audio file
    bool encode_speaker(const std::string & reference_audio,
                        std::vector<float> & embedding);

    // Generate speech with a pre-computed speaker embedding
...
#ifdef QWEN3_TTS_TENSORRT
    TRTVocoderDecoder trt_decoder_;
    bool trt_decoder_loaded_ = false;
#endif
